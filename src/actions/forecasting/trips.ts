...
export async function importTrips(
  ...
  try {
      ...
      if (existing) {
        ...
        await prisma.trip.update({
          ...
          data: {
            ...
            estimatedAccessorial: tripData.estimatedAccessorial,
            projectedRevenue: tripData.projectedRevenue,
          },
        });

        // Delete old loads and create new ones
        await prisma.tripLoad.deleteMany({ where: { tripDbId: existing.id } });
        await prisma.tripLoad.createMany({
          data: tripData.loads.map((load) => ({
            tripDbId: existing.id,
            loadId: load.loadId,
        ...
        await prisma.trip.create({
          data: {
            ...
            loads: {
              create: tripData.loads.map((load) => ({
                loadId: load.loadId,
                facilitySequence: load.facilitySequence || null,
                loadExecutionStatus: load.loadExecutionStatus,
                truckFilter: load.truckFilter || null,
    ...
    // Update ForecastWeeks for the affected weeks
    ...
    for (const weekStartTime of uniqueWeekStarts) {
      await createOrUpdateForecastWeek(session.user.id, new Date(weekStartTime));
    }
