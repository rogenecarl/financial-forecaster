...
export function TripsImportModal({
  open,
  onOpenChange,
  onSuccess,
  batchId,
  ...
  const processFile = useCallback(async (selectedFile: File, fileHash: string, skipDuplicateCheck = false) => {
    ...
    if (!skipDuplicateCheck) {
      ...
      const duplicateCheck = await checkDuplicateTripFile(batchId, fileHash);
    ...
    const parsed = await parseTripsFile(selectedFile);
    setParseResult(parsed);
    setProgress(50);

    if (!parsed.success || parsed.trips.length === 0) {
      setError(parsed.errors.join(", ") || "No valid trips found in file");
      setStage("error");
      return;
    }
    ...
    if (result.success && result.data) {
      ...
      if (result.error?.startsWith("DUPLICATE_FILE:")) {
        ...
        setError(result.error || "Failed to import trips");
        setStage("error");
      }
    }
  }, [batchId, mode]);
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-medium">
                    {stage === "parsing" ? "Parsing file..." : "Importing trips..."}
                  </span>
                </div>
            ...
            {stage === "complete" && !showResultDialog && (
              <Alert className="border-emerald-200 bg-emerald-50">
                ...
                <AlertDescription className="text-emerald-700">
                  Successfully imported trips to the batch.
                </AlertDescription>
              </Alert>
            )}
          </div>
